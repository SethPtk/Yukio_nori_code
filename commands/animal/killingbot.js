            ...
            if(args[0] && !userData.autohunt.autohunting){
                if(args[0] == 'set'){
                    ...
                    if(amount){
                        if(amount >= 500){
                            showTime = '5h';
                            time_in_ms = 18_000_000;

                            amount = 500;
                            spend = 25000000;
                        }else if(amount >= 400){
                            showTime = '4h';
                            time_in_ms = 14_400_000;

                            amount = 400;
                            spend = 20000000;
                        }else if(amount >= 300){
                            showTime = '3h';
                            time_in_ms = 10_800_000;

                            amount = 300;
                            spend = 15000000;
                        }else if(amount >= 200){
                            showTime = '2h';
                            time_in_ms = 7_200_000;

                            spend = 10000000;
                            amount = 200;
                        }else if(amount >= 100){
                            showTime = '1h';
                            time_in_ms = 3_600_000;

                            spend = 5000000;
                            amount = 100;
                        }else{
                           return message.channel.send({ embeds: [SimpleEmbed(`<@${user.id}> Killing amount must be 100 or higher!`)] });
                        }
                ...
                if(currentTime >= userData.autohunt.datekilling && userData.autohunt.autohunting){
                    ...
                    for(let i = 1; i <= userData.autohunt.amount_animal; i++){
                        const ran_rank = getRandomInt(1, 12);
                        if(ran_rank == 1){
                            rank_c += 1;
