...
module.exports = {
    ...
    async execute(client, message, args){
        try{
            ...
            if(userData.quest.hunt_point < 50){
                userData.quest.hunt_point += 1;
            }
            
            if(userData.balance <= 0){ message.reply({ embeds: [SimpleEmbed(`<@${user.id}>** You don't have enough cash!**`)] }); return; }else{ userData.balance -= 5; }
            const mgs = await message.channel.send({ embeds: [SimpleEmbed(`<@${user.id}> hunting...`)] });
            const Sat = userData.sat;
            const Gem = userData.gem;
            let chance = userData.gem.lucky_Gem_addChance;
            ...
            if(!Gem.hunt_Gem_equipe && !Gem.empowering_Gem_equipe && !Gem.lucky_Gem_equipe){
                messageSat = `**Now <@${user.id}>**, You caught a `;
            }else{
                if(Gem.hunt_Gem_equipe != '' && Gem.hunt_Gem_addAnimal < 0){ Gem.hunt_Gem_equipe = ''; await userData.save(); }
                let show_hunt_gem = '';
                if(Gem.hunt_Gem_equipe){
                    ...
                    if(Gem.hunt_Gem_percen < 0){ Gem.hunt_Gem_equipe = ''; Gem.hunt_Gem_addAnimal = 1; }else{ Gem.hunt_Gem_percen -= gif[`${Gem.hunt_Gem_equipe}_minus`]; }
                }

                if(Gem.empowering_Gem_equipe != '' && Gem.empowering_Gem_timeAniaml < 0){ Gem.empowering_Gem_equipe = ''; await userData.save(); }
                ...
                if(Gem.empowering_Gem_equipe){
                    ...
                    if(Gem.empowering_Gem_percen < 0){ Gem.empowering_Gem_equipe = ''; Gem.empowering_Gem_timeAniaml = 1; }else{ Gem.empowering_Gem_percen -= gif[`${Gem.empowering_Gem_equipe}_minus`]; }
                }

                if(Gem.lucky_Gem_equipe != '' && Gem.lucky_Gem_addChance < 0){ Gem.lucky_Gem_equipe = ''; await userData.save(); }
                let show_lucky_gem = ''
                if(Gem.lucky_Gem_equipe){
                    ...
                    if(Gem.lucky_Gem_percen < 0){ Gem.lucky_Gem_equipe = ''; Gem.lucky_Gem_addChance = 1; }else{ Gem.lucky_Gem_percen -= gif[`${Gem.lucky_Gem_equipe}_minus`]; }
                }

                amount_of_get = (Gem.hunt_Gem_addAnimal*Gem.empowering_Gem_timeAniaml);
                messageSat = `**Now <@${user.id}>**, you using gems ${show_hunt_gem}${show_empowering_gem}${show_lucky_gem}!\nYou got animal like: `;
            }
            ...
            for(let i = 1; i <= amount_of_get; i++){
                ...
                if(luck_hunt == 1){
                    ...
                    top_ran = 7;
                }else if(luck_hunt > 5 && luck_hunt <= 10){
                    top_ran = 6;
                }else if(luck_hunt > 10 && luck_hunt <= 30){
                    top_ran = 5;
                }else if(luck_hunt > 30 && luck_hunt <= 60){
                    top_ran = 4;
                }else if(luck_hunt > 60 && luck_hunt <= 100){
                    top_ran = 3;
                }

                if(top_ran == 4){//check epic lucky
                ...
                }else if(top_ran == 5){//check mythical lucky
                ...
                }else if(top_ran == 6){//check legendary lucky
                ...
                }else if(top_ran == 7){//check gem lucky
                ...
                }else if(top_ran == 8){//check febled lucky
            ...
            if(Sat.team.team_equipe1 || Sat.team.team_equipe2 || Sat.team.team_equipe3){ messageSat += `\n\nYour team **[ `; }
            if(Sat.team.team_equipe1){
                const postion1_name = getAnimalNameByName(Sat.team.team_equipe1)
                if(postion1_name){ messageSat += `${gif[`rank_${getAnimalIdByName(postion1_name)}`]} `}
            ...
            if(Sat.team.team_equipe2){
                ...
                if(postion2_name){ messageSat += `${gif[`rank_${getAnimalIdByName(postion2_name)}`]} `}
            ...
            if(Sat.team.team_equipe3){
                ...
                if(postion3_name){ messageSat += `${gif[`rank_${getAnimalIdByName(postion3_name)}`]} `}
                Sat[`sat_${getAnimalIdByName(postion3_name)}_xp`] += gainAnimalXp;
            }
            if(Sat.team.team_equipe1 || Sat.team.team_equipe2 || Sat.team.team_equipe3){ messageSat += `]** xp up **${gainAnimalXp}**!`; }

            mgs.edit({ embeds: [SimpleEmbed(messageSat)] });
