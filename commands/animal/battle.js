...
module.exports = {
    ...
    async execute(client, message, args){
        try {
            ...
            if(choice){
                if(choice == 'set'){
                    ...
                    if(choice_two == 'normal'){
                        ...
                        message.channel.send({ embeds: [SimpleEmbed(`<@${user.id}> option battle has change to **NORMAL**`)] });
                        try{ await userData.save(); }catch(error){}
                        return;
                    }else if(choice_two == 'fast'){
                        userData.sat.team.setting_battle = 'fast';
                        message.channel.send({ embeds: [SimpleEmbed(`<@${user.id}> option battle has change to **FAST**`)] });
                        try{ await userData.save(); }catch(error){}
                        return;
                    }
                }
            ...
            if(userData.quest.battle_point < 50){
            ...
            let randomIndex = getAllUserIds()[Math.floor(Math.random() * getAllUserIds().length)];
            while(randomIndex == user.id){
                randomIndex = getAllUserIds()[Math.floor(Math.random() * getAllUserIds().length)]; 
            }
            ...
            if(mention){
                ...
                if(!target){ return message.reply({ embeds: [SimpleEmbed(`<@${mention.id}> didn't play Yukio`)] }); }
                if(!target.sat.team.team_equipe1 && !target.sat.team.team_equipe2 && !target.sat.team.team_equipe3){ mgs.edit('This user do not has animal!'); return; }
                ...
                const embed = customEmbed()
                    .setAuthor({ name: `${mention.displayName}, ${user.displayName} challenges you to a duel!`, iconURL: user.displayAvatarURL() })
                    .setColor('Aqua')
                    .setDescription(`${sym}ab${sym} to accept the battle!\n${sym}db${sym} to decline the battle!`)
                    .setTimestamp()
                const agree_button = labelButton('agree_button', 'Accept', ButtonStyle.Success);
                ...
                collector.on('collect', async (interaction) =>{
                    ...
                    if(interaction.customId == 'agree_button'){
                        await interaction.update({ embeds: [SimpleEmbed('Starting challenges a duel...')], components: [] });
                        randomIndex = mention.id;
                        startDuel = true;
                    }
                    if(interaction.customId == 'decline_button'){
                        mgs.edit({ embeds: [SimpleEmbed(`<@${interaction.member.user.id}> has decline challenges you to a duel!`)], components: [] });
                        collector.stop();
                        return;
                    }
                });
            ...
            if(Sat.team.team_equipe1){
                ...
                for(const allwp of userData.wp){
                    ...
                    if(boolStr.toLowerCase() == Sat.team.team_equipe1.toLowerCase()){
                        ...
                        sat1 = activeWeapon(sat1, name, passive, percen);
            ...
            if(Sat.team.team_equipe2){
                ...
                for(const allwp of userData.wp){
                    ...
                    if(boolStr.toLowerCase() == Sat.team.team_equipe2.toLowerCase()){
                        item = ` - ${getRank(rank)}${getWeaponRank(name, rank)}${getPassive(passive)}${passive_two_gif}`;
                        sat2 = activeWeapon(sat2, name, passive, percen);
            ...
            if(Sat.team.team_equipe3){
                ...
                for(const allwp of userData.wp){
                    ...
                    if(boolStr.toLowerCase() == Sat.team.team_equipe3.toLowerCase()){
                        item = ` - ${getRank(rank)}${getWeaponRank(name, rank)}${getPassive(passive)}${passive_two_gif}`;
                        sat3 = activeWeapon(sat3, name, passive, percen);
            ...
            if(oppoData && oppoData.sat.team.team_equipe1 || oppoData.sat.team.team_equipe2 || oppoData.sat.team.team_equipe3){
                ...
                if(oppoData.sat.team.team_equipe1 != ''){
                    ...
                    for(const allwp of oppoData.wp){
                        ...
                        if(boolStr.toLowerCase() == oppoData.sat.team.team_equipe1.toLowerCase()){
                            item = ` - ${getRank(rank)}${getWeaponRank(name, rank)}${getPassive(passive)}${passive_two_gif}`;
                            ene1 = activeWeapon(ene1, name, passive, percen);
                ...
                if(oppoData.sat.team.team_equipe2 != ''){
                    ...
                    for(const allwp of oppoData.wp){
                        ...
                        if(boolStr.toLowerCase() == oppoData.sat.team.team_equipe2.toLowerCase()){
                            item = ` - ${getRank(rank)}${getWeaponRank(name, rank)}${getPassive(passive)}${passive_two_gif}`;
                            ene2 = activeWeapon(ene2, name, passive, percen);
                ...
                if(oppoData.sat.team.team_equipe3 != ''){
                    ...
                    for(const allwp of oppoData.wp){
                        ...
                        if(boolStr.toLowerCase() == oppoData.sat.team.team_equipe3.toLowerCase()){
                            item = ` - ${getRank(rank)}${getWeaponRank(name, rank)}${getPassive(passive)}${passive_two_gif}`;
                            ene3 = activeWeapon(ene3, name, passive, percen);
                ...
                if(enemy_ran == 1){
                    const P_id = getAnimalIdByName(gif[`rank_${team_ran}_${animal_ran}_name`]);

                    ene1.png = `${gif[`rank_${P_id}_png`]}`;
                    ene1.name = getAnimalNameByName(gif[`rank_${team_ran}_${animal_ran}_name`]);
                    ...
                    messageEnemyTeam += `L. ${lvl_ran} ${gif[`rank_${team_ran}_${animal_ran}`]} - no weapon\n`;
                    enes += 1;
                    eneAlllvl += lvl_ran;
                }else if(enemy_ran == 2){
                    const P_id = getAnimalIdByName(gif[`rank_${team_ran}_${animal_ran}_name`]);
                    ...
                    const P_id2 = getAnimalIdByName(gif[`rank_${team_ran2}_${animal_ran2}_name`]);

                    ene2.png = `${gif[`rank_${P_id2}_png`]}`;
                    ene2.name = getAnimalNameByName(gif[`rank_${team_ran2}_${animal_ran2}_name`]);
                    ...
                    messageEnemyTeam += `L. ${lvl_ran} ${gif[`rank_${team_ran}_${animal_ran}`]} - no weapon\n`;
                    messageEnemyTeam += `L. ${lvl_ran2} ${gif[`rank_${team_ran2}_${animal_ran2}`]} - no weapon\n`;
                    enes += 2;
                    ...
                    const P_id3 = getAnimalIdByName(gif[`rank_${team_ran3}_${animal_ran3}_name`]);

                    ene3.png = `${gif[`rank_${P_id3}_png`]}`;
                    ene3.name = getAnimalNameByName(gif[`rank_${team_ran3}_${animal_ran3}_name`]);
                    ...
                    messageEnemyTeam += `L. ${lvl_ran} ${gif[`rank_${team_ran}_${animal_ran}`]} - no weapon\n`;
                    messageEnemyTeam += `L. ${lvl_ran2} ${gif[`rank_${team_ran2}_${animal_ran2}`]} - no weapon\n`;
                    messageEnemyTeam += `L. ${lvl_ran3} ${gif[`rank_${team_ran3}_${animal_ran3}`]} - no weapon\n`;
                    enes += 3;
            ...
            if(crate_ran == 1 && userData.daily_crate <= 4){
                ...
                message.channel.send(`You found ${gif['100']}**crate**! ${sym}[${userData.daily_crate}/5] will reset in ${hours}h, ${minutes}m, ${seconds}s${sym}`);
                userData.gem['100'] += 1;
            }
            ...
            if(userData.sat.team.setting_battle == 'normal'){
                ...
                const embed = customEmbed()
                    .setAuthor({ name: `${user.displayName} goes into battle!`,iconURL: user.displayAvatarURL() })
                    .setColor('#8EC3FF')
                    .setImage(`attachment://sat1.png`)
                    .addFields(
                        { name: `${ourTeamName}`, value: `${messageOurTeam}`, inline: true },
                        { name: `${enemyTeamName}`, value: `${messageEnemyTeam}`, inline: true },
                    )
                ...
                if(sat1.weapon_bool && sat1.hp > 0){
                    for(const allwp of userData.wp){
                        ...
                        if(boolStr.toLowerCase() == Sat.team.postion1.toLowerCase()){
                            sat1 = battleWithWeapon(sat1, name, passive, passive_two);
                        }
                    }
                }
                if(sat2.weapon_bool && sat2.hp > 0){
                    for(const allwp of userData.wp){
                        ...
                        if(boolStr.toLowerCase() == Sat.team.postion2.toLowerCase()){
                            sat2 = battleWithWeapon(sat2, name, passive, passive_two);
                        }
                    }
                }
                if(sat3.weapon_bool && sat3.hp > 0){
                    for(const allwp of userData.wp){
                        ...
                        if(boolStr.toLowerCase() == Sat.team.postion3.toLowerCase()){
                            sat3 = battleWithWeapon(sat3, name, passive, passive_two);
                        }
                    }
                }
                ...
                if(ene1.weapon_bool && ene1.hp > 0){
                    for(const allwp of oppoData.wp){
                        ...
                        if(boolStr.toLowerCase() == oppoData.sat.team.postion1.toLowerCase()){
                            ene1 = battleWithWeapon(ene1, name, passive, passive_two);
                        }
                    }
                }
                if(ene2.weapon_bool && ene2.hp > 0){
                    for(const allwp of oppoData.wp){
                        ...
                        if(boolStr.toLowerCase() == oppoData.sat.team.postion2.toLowerCase()){
                            ene2 = battleWithWeapon(ene2, name, passive, passive_two);
                        }
                    }
                }
                if(ene3.weapon_bool && ene3.hp > 0){
                    for(const allwp of oppoData.wp){
                        ...
                        if(boolStr.toLowerCase() == oppoData.sat.team.postion3.toLowerCase()){
                            ene3 = battleWithWeapon(ene3, name, passive, passive_two);
                        }
                    }
                }
            ...
            if(!mention){
                ...
                if((sat1.hp == 0 && sat2.hp == 0 && sat3.hp == 0 && ene1.hp == 0 && ene2.hp == 0 && ene3.hp == 0) || (round >= end_round)){
                    bonus = 100;
                    ...
                    .setFooter({ text: `Both ties in ${round} turns! Your team gained 100 xp! streak of ${streak} wins...` })
                    ...
                    .setFooter({ text: `You lost in ${round} turns! Your team gained 50 xp! You lost your streak of ${streak} wins...` })
                    mgs.edit({ embeds: [result], files: [attachment] });
                    streak = 0;
                    if(userData.sat.team.team_set == 1){
                    ...
                    if(userData.sat.team.team_set == 1){
                        ...
                        if(userData.sat.team.streak_two > userData.sat.team.higher_streak_two){
                ...
                if(userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe1)}_xp`] > userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe2)}_xp`] && userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe1)}_xp`] > userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe3)}_xp`]){
                ...
                }else if(userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe2)}_xp`] > userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe1)}_xp`] && userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe2)}_xp`] > userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe3)}_xp`]){
                    const P_id = getAnimalIdByName(userData.sat.team.team_equipe2);
                    const xp = parseInt(userData.sat[`sat_${P_id}_xp`]);
                    highLvl = xpToLevel(xp);
                }else if(userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe3)}_xp`] > userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe1)}_xp`] && userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe3)}_xp`] > userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe2)}_xp`]){
                ...
                if(userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe1)}_xp`] < userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe2)}_xp`] && userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe1)}_xp`] < userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe3)}_xp`]){
                    userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe1)}_xp`] += parseInt(200*highLvl);
                }else if(userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe2)}_xp`] < userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe1)}_xp`] && userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe2)}_xp`] < userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe3)}_xp`]){
                    userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe2)}_xp`] += parseInt(200*highLvl);
                }else if(userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe3)}_xp`] < userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe1)}_xp`] && userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe3)}_xp`] < userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe2)}_xp`]){
                    userData.sat[`sat_${getAnimalIdByName(userData.sat.team.team_equipe3)}_xp`] += parseInt(200*highLvl);
                }

                if(userData.sat.team.team_equipe1){ const postion1_name = getAnimalNameByName(userData.sat.team.team_equipe1); userData.sat[`sat_${getAnimalIdByName(postion1_name)}_xp`] += bonus+200; }
                if(userData.sat.team.team_equipe2){ const postion2_name = getAnimalNameByName(userData.sat.team.team_equipe2); userData.sat[`sat_${getAnimalIdByName(postion2_name)}_xp`] += bonus+200; }
                if(userData.sat.team.team_equipe3){ const postion3_name = getAnimalNameByName(userData.sat.team.team_equipe3); userData.sat[`sat_${getAnimalIdByName(postion3_name)}_xp`] += bonus+200; }
