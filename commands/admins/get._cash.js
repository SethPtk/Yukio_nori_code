    ...
    async execute(client, message, args) {
        ...
        if(mentions){
            ...
            if(target){
                ...
                await target.save();
                message.channel.send({ embeds: [SimpleEmbed(`**${mentions.displayName}** got ${gif.cash} **${amount.toLocaleString()}**$!!!`)] });
                return;
            }else{
                message.channel.send({ embeds: [SimpleEmbed(`**${mentions.displayName}** did not play Yukio!`)] });
            }
        }
