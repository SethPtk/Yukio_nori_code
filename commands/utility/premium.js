    ...
    async execute(client, message, args) {
        try{
            ...
            if(userData.premium.premium_bool == true){
                ...
                message.channel.send({ embeds: [SimpleEmbed(`Pe: **${formattedTimestamp}** left\n${message_timeleft}\nAs: **PREMIUM YUKIO** ${gif.premium_yukio}\n\n**Include**:\n${gif.arrow_premium}**Limit Tranfer x2**\n${gif.arrow_premium}**All cmd CD -50%**\n${gif.arrow_premium}**Message[CMD] React ${gif.premium_yukio}**\n\n**Daily claim**:\n${gif.arrow_premium}${gif.cash}**500,000**$\n${gif.arrow_premium}${gif.fire_gif} **1**\n${gif.arrow_premium}${gif.zaz_gif} **1**\n${gif.arrow_premium}${gif['050']} **5**\n${gif.arrow_premium}${gif['100']} **5**\n\n**Note**: do not forget to claim your daily Reward!`).setAuthor({ name: `${user.displayName}'s Premium`, iconURL: user.displayAvatarURL() })] });
                return;
            }
